...
```bash
kubectl create ns ns-remote-nsmgr-restart-ip
```
...
resources:
- https://raw.githubusercontent.com/networkservicemesh/deployments-k8s/90929a4302a03a46d22e303c375c488f9336693e/examples/heal/remote-nsmgr-restart-ip/netsvc.yaml
        ...
        - name: nsc
          env:
            - name: NSM_NETWORK_SERVICES
              value: kernel://remote-nsmgr-restart-ip/nsm-1
      nodeName: ${NODES[0]}
EOF
...
```bash
kubectl wait --for=condition=ready --timeout=1m pod -l app=nsc-kernel -n ns-remote-nsmgr-restart-ip
```
```bash
...
```bash
NSC=$(kubectl get pods -l app=nsc-kernel -n ns-remote-nsmgr-restart-ip --template '{{range .items}}{{.metadata.name}}{{"\n"}}{{end}}')
```
```bash
