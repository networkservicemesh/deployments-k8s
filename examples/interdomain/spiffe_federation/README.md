...
```bash
kubectl --kubeconfig=$KUBECONFIG1 apply -f https://raw.githubusercontent.com/networkservicemesh/deployments-k8s/03cd896de180655961983276d0dd90a6d0bd89be/examples/interdomain/spiffe_federation/cluster1-spiffeid-template.yaml
```

Apply the ClusterSPIFFEID CR for the second cluster:
```bash
kubectl --kubeconfig=$KUBECONFIG2 apply -f https://raw.githubusercontent.com/networkservicemesh/deployments-k8s/03cd896de180655961983276d0dd90a6d0bd89be/examples/interdomain/spiffe_federation/cluster2-spiffeid-template.yaml
```
