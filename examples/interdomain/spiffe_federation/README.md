...
```bash
kubectl --kubeconfig=$KUBECONFIG1 apply -f https://raw.githubusercontent.com/networkservicemesh/deployments-k8s/9eb84310ffd7aaf7a7b8d0e38f8a8ef5e4cd52e5/examples/interdomain/spiffe_federation/cluster1-spiffeid-template.yaml
```

Apply the ClusterSPIFFEID CR for the second cluster:
```bash
kubectl --kubeconfig=$KUBECONFIG2 apply -f https://raw.githubusercontent.com/networkservicemesh/deployments-k8s/9eb84310ffd7aaf7a7b8d0e38f8a8ef5e4cd52e5/examples/interdomain/spiffe_federation/cluster2-spiffeid-template.yaml
```
