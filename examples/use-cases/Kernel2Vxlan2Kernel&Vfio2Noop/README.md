...
resources: 
- https://github.com/networkservicemesh/deployments-k8s/examples/use-cases/Kernel2Vxlan2Kernel&Vfio2Noop?ref=3d1dcfe1de90681213c7f0006f25279bb4699966
...
- https://github.com/networkservicemesh/deployments-k8s/apps/nsc-vfio?ref=b3b9066d54b23eee85de6a5b1578c7b49065fb89
- https://github.com/networkservicemesh/deployments-k8s/apps/nse-vfio?ref=b3b9066d54b23eee85de6a5b1578c7b49065fb89
        ...
        - name: nsc
          env:
            - name: NSM_NETWORK_SERVICES
              value: kernel://kernel2vxlan2kernel-vfio2noop/nsm-1
      nodeName: ${NODES[0]}
EOF
```
        ...
        - name: nse
          env:
            ...
            - name: NSM_SERVICE_NAMES
              value: "kernel2vxlan2kernel-vfio2noop"
...
```bash
kubectl wait --for=condition=ready --timeout=1m pod -l app=nsc-kernel -n ns-kernel2vxlan2kernel-vfio2noop
```
```bash
...
```bash
NSC_KERNEL=$(kubectl get pods -l app=nsc-kernel -n ns-kernel2vxlan2kernel-vfio2noop --template '{{range .items}}{{.metadata.name}}{{"\n"}}{{end}}')
```
```bash
