...
```bash
NAMESPACE=($(kubectl create -f https://raw.githubusercontent.com/networkservicemesh/deployments-k8s/d07ffd38237baa345b85417b8a3405a464efa1e4/examples/features/namespace.yaml)[0])
...
bases:
- https://github.com/networkservicemesh/deployments-k8s/apps/nse-supplier-k8s?ref=d07ffd38237baa345b85417b8a3405a464efa1e4
- https://github.com/networkservicemesh/deployments-k8s/apps/nsc-kernel?ref=d07ffd38237baa345b85417b8a3405a464efa1e4
...
configMapGenerator:
  - name: supplier-pod-template-configmap
    files:
      - https://raw.githubusercontent.com/networkservicemesh/deployments-k8s/d07ffd38237baa345b85417b8a3405a464efa1e4/examples/features/scale-from-zero/pod-template.yaml
EOF
```

6. Register network service:
```bash
kubectl apply -f https://raw.githubusercontent.com/networkservicemesh/deployments-k8s/d07ffd38237baa345b85417b8a3405a464efa1e4/examples/features/scale-from-zero/autoscale-netsvc.yaml
```
