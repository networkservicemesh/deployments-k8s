...
```bash
NAMESPACE=($(kubectl create -f https://raw.githubusercontent.com/networkservicemesh/deployments-k8s/57c9003b1048888a88cd734cfd769f0cd22c7011/examples/features/namespace.yaml)[0])
...
bases:
- https://github.com/networkservicemesh/deployments-k8s/apps/nse-supplier-k8s?ref=57c9003b1048888a88cd734cfd769f0cd22c7011
- https://github.com/networkservicemesh/deployments-k8s/apps/nsc-kernel?ref=57c9003b1048888a88cd734cfd769f0cd22c7011
...
configMapGenerator:
  - name: supplier-pod-template-configmap
    files:
      - https://raw.githubusercontent.com/networkservicemesh/deployments-k8s/57c9003b1048888a88cd734cfd769f0cd22c7011/examples/features/scale-from-zero/pod-template.yaml
EOF
```

6. Register network service:
```bash
kubectl apply -f https://raw.githubusercontent.com/networkservicemesh/deployments-k8s/57c9003b1048888a88cd734cfd769f0cd22c7011/examples/features/scale-from-zero/autoscale-netsvc.yaml
```
