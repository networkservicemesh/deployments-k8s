...
```bash
NAMESPACE=($(kubectl create -f https://raw.githubusercontent.com/networkservicemesh/deployments-k8s/5d3bf1cd22c07e2d9b4d0ef673214d29822fa571/examples/features/namespace.yaml)[0])
...
bases:
- https://github.com/networkservicemesh/deployments-k8s/apps/nse-supplier-k8s?ref=5d3bf1cd22c07e2d9b4d0ef673214d29822fa571
- https://github.com/networkservicemesh/deployments-k8s/apps/nsc-kernel?ref=5d3bf1cd22c07e2d9b4d0ef673214d29822fa571
...
configMapGenerator:
  - name: supplier-pod-template-configmap
    files:
      - https://raw.githubusercontent.com/networkservicemesh/deployments-k8s/5d3bf1cd22c07e2d9b4d0ef673214d29822fa571/examples/features/scale-from-zero/pod-template.yaml
EOF
```

6. Register network service:
```bash
kubectl apply -f https://raw.githubusercontent.com/networkservicemesh/deployments-k8s/5d3bf1cd22c07e2d9b4d0ef673214d29822fa571/examples/features/scale-from-zero/autoscale-netsvc.yaml
```
