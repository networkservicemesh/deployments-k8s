...
```bash
NAMESPACE=($(kubectl create -f https://raw.githubusercontent.com/networkservicemesh/deployments-k8s/v1.5.0-rc.2/examples/features/namespace.yaml)[0])
...
bases:
- https://github.com/networkservicemesh/deployments-k8s/apps/nse-supplier-k8s?ref=v1.5.0-rc.2
- https://github.com/networkservicemesh/deployments-k8s/apps/nsc-kernel?ref=v1.5.0-rc.2
...
configMapGenerator:
  - name: supplier-pod-template-configmap
    files:
      - https://raw.githubusercontent.com/networkservicemesh/deployments-k8s/v1.5.0-rc.2/examples/features/scale-from-zero/pod-template.yaml
EOF
```

6. Register network service:
```bash
kubectl apply -f https://raw.githubusercontent.com/networkservicemesh/deployments-k8s/v1.5.0-rc.2/examples/features/scale-from-zero/autoscale-netsvc.yaml
```
