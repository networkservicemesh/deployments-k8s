...
grep '?ref=39475d6e3810fabc1b45eb6104847b9a89988c05' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=39475d6e3810fabc1b45eb6104847b9a89988c05/${root}\/\2/g" "${file}"
done
