...
grep 'ref=294af207196411f4489d7f58513bcdf24926e2f5' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=294af207196411f4489d7f58513bcdf24926e2f5[a-z0-9]*/${root}\/\2/g" "${file}"
done
