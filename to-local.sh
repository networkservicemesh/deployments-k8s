...
grep '?ref=4a9f36656a74cea51a9df810f9a5884534195262' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=4a9f36656a74cea51a9df810f9a5884534195262/${root}\/\2/g" "${file}"
done
