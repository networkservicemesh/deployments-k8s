...
grep 'ref=be7438e8669355a05ec87784e5bd9bd5dfa17422' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=be7438e8669355a05ec87784e5bd9bd5dfa17422[a-z0-9]*/${root}\/\2/g" "${file}"
done
