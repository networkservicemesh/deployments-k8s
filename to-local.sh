...
grep 'ref=dc3653bc8ff44c685d0ea17f58f32d506529652d' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=dc3653bc8ff44c685d0ea17f58f32d506529652d[a-z0-9]*/${root}\/\2/g" "${file}"
done
