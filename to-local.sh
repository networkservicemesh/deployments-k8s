...
grep 'ref=004180adbd98215a584bff0387506e8e085ee75b' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=004180adbd98215a584bff0387506e8e085ee75b[a-z0-9]*/${root}\/\2/g" "${file}"
done
