...
grep 'ref=6e1f8c2b189158a01e007f4e308247cc7261ff0b' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=6e1f8c2b189158a01e007f4e308247cc7261ff0b[a-z0-9]*/${root}\/\2/g" "${file}"
done
