...
grep 'ref=a852347fbfd1c3c6b845580c16933872350a8530' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=a852347fbfd1c3c6b845580c16933872350a8530[a-z0-9]*/${root}\/\2/g" "${file}"
done
