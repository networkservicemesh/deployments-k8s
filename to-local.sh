...
grep 'ref=b613c1ee980371850561640b3a152d6a713d09ff' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=b613c1ee980371850561640b3a152d6a713d09ff[a-z0-9]*/${root}\/\2/g" "${file}"
done
