...
grep 'ref=65627f72568f2be0b006ee4dea75566df084681d' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=65627f72568f2be0b006ee4dea75566df084681d[a-z0-9]*/${root}\/\2/g" "${file}"
done
