...
grep 'ref=5013e8db94a583036b9410be8628914109c5234d' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=5013e8db94a583036b9410be8628914109c5234d[a-z0-9]*/${root}\/\2/g" "${file}"
done
