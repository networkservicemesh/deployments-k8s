...
grep 'ref=7e3b3e3e5e3e42fc5e309a730e39921791754570' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=7e3b3e3e5e3e42fc5e309a730e39921791754570[a-z0-9]*/${root}\/\2/g" "${file}"
done
