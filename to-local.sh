...
grep 'ref=357d8f0a888265abfd5327e4eb553b62c943b4b4' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=357d8f0a888265abfd5327e4eb553b62c943b4b4[a-z0-9]*/${root}\/\2/g" "${file}"
done
