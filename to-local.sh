...
grep 'ref=77ac83abb354f79685538b575c133bf45c00a68a' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=77ac83abb354f79685538b575c133bf45c00a68a[a-z0-9]*/${root}\/\2/g" "${file}"
done
