...
grep '?ref=c9152deec71d48890bfa201eb7e381203eb6cb58' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=c9152deec71d48890bfa201eb7e381203eb6cb58/${root}\/\2/g" "${file}"
done
