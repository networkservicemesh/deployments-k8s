...
grep 'ref=8696ff93ef1d0c89a19a6f5fe2134f5409a1a974' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=8696ff93ef1d0c89a19a6f5fe2134f5409a1a974[a-z0-9]*/${root}\/\2/g" "${file}"
done
