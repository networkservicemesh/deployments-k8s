...
grep 'ref=96b94bdf8f0c7f053654f750381d9c4cd0e8049d' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=96b94bdf8f0c7f053654f750381d9c4cd0e8049d[a-z0-9]*/${root}\/\2/g" "${file}"
done
