...
grep 'ref=cb4749df0911e135457081d7616a0c33fcb6a0e4' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=cb4749df0911e135457081d7616a0c33fcb6a0e4[a-z0-9]*/${root}\/\2/g" "${file}"
done
