...
grep 'ref=d27aeb8686f26b9cd4026eb42047d1e8eecbdc39' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=d27aeb8686f26b9cd4026eb42047d1e8eecbdc39[a-z0-9]*/${root}\/\2/g" "${file}"
done
