...
grep 'ref=v1.1.1-rc.3' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=v1.1.1-rc.3[a-z0-9]*/${root}\/\2/g" "${file}"
done
