...
grep 'ref=dc404f0e40d6198e9ac7c95633483af7436421c9' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=dc404f0e40d6198e9ac7c95633483af7436421c9[a-z0-9]*/${root}\/\2/g" "${file}"
done
