...
grep '?ref=6e523c7667efafbf2d01baa374a4ef0408997823' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=6e523c7667efafbf2d01baa374a4ef0408997823/${root}\/\2/g" "${file}"
done
