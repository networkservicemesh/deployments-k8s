...
grep '?ref=26fcc6052ccebc029b37e5540d77006811441c33' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=26fcc6052ccebc029b37e5540d77006811441c33/${root}\/\2/g" "${file}"
done
