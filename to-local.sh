...
grep 'ref=6f244676e6f11a54a30f09471cc6db83f9cbb947' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=6f244676e6f11a54a30f09471cc6db83f9cbb947[a-z0-9]*/${root}\/\2/g" "${file}"
done
