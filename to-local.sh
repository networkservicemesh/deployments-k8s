...
grep '?ref=106c5ecbdfc3b5a622644241980a05fdb4ea8820' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=106c5ecbdfc3b5a622644241980a05fdb4ea8820/${root}\/\2/g" "${file}"
done
