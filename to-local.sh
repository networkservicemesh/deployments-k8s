...
grep 'ref=7481bbed2a65b138d804c186687005861bdf4f8f' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=7481bbed2a65b138d804c186687005861bdf4f8f[a-z0-9]*/${root}\/\2/g" "${file}"
done
