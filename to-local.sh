...
grep 'ref=2a461ef182d7938910b2b2220404f964961ee9bc' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=2a461ef182d7938910b2b2220404f964961ee9bc[a-z0-9]*/${root}\/\2/g" "${file}"
done
