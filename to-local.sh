...
grep 'ref=c1c942e260127da4cc742c2d1b7c69e1a7ed6e8d' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=c1c942e260127da4cc742c2d1b7c69e1a7ed6e8d[a-z0-9]*/${root}\/\2/g" "${file}"
done
