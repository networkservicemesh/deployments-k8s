...
grep '?ref=c2123339a6f8491b6e9b00837e4e7560f1d10073' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=c2123339a6f8491b6e9b00837e4e7560f1d10073/${root}\/\2/g" "${file}"
done
