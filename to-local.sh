...
grep 'ref=dbdb040ab5959c8c3dc26347978367677466e749' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=dbdb040ab5959c8c3dc26347978367677466e749[a-z0-9]*/${root}\/\2/g" "${file}"
done
