...
grep 'ref=1a519a10582038b00e5d1206db6342172519abc1' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=1a519a10582038b00e5d1206db6342172519abc1[a-z0-9]*/${root}\/\2/g" "${file}"
done
