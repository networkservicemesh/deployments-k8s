...
grep 'ref=b63fd557f2f328721390d734783f07300f3e4058' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=b63fd557f2f328721390d734783f07300f3e4058[a-z0-9]*/${root}\/\2/g" "${file}"
done
