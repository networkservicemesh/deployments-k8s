...
grep 'ref=3268f386c2af63cea7eb8428040279de2441a035' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=3268f386c2af63cea7eb8428040279de2441a035[a-z0-9]*/${root}\/\2/g" "${file}"
done
