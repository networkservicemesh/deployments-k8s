...
grep '?ref=1342002d2a9e1d8258cad991ff38390329c85252' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=1342002d2a9e1d8258cad991ff38390329c85252/${root}\/\2/g" "${file}"
done
