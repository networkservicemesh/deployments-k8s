...
grep '?ref=91e80e8e4531562720d50ea9e84a82299813db65' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=91e80e8e4531562720d50ea9e84a82299813db65/${root}\/\2/g" "${file}"
done
