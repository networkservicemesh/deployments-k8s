...
grep 'ref=8a42d7ab4006f70e9e3e9aabc38c97283a3b4aeb' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=8a42d7ab4006f70e9e3e9aabc38c97283a3b4aeb[a-z0-9]*/${root}\/\2/g" "${file}"
done
