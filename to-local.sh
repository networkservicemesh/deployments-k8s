...
grep 'ref=f1b9aa6e760b2c9737444f5b1da8746346cd276b' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=f1b9aa6e760b2c9737444f5b1da8746346cd276b[a-z0-9]*/${root}\/\2/g" "${file}"
done
