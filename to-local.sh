...
grep '?ref=4f0a564767a1d8cb1a2382c2dcef960008ca094d' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=4f0a564767a1d8cb1a2382c2dcef960008ca094d/${root}\/\2/g" "${file}"
done
