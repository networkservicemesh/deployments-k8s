...
grep '?ref=df79cd279fa0e2a7e6403281c92bcffa1666a1eb' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=df79cd279fa0e2a7e6403281c92bcffa1666a1eb/${root}\/\2/g" "${file}"
done
