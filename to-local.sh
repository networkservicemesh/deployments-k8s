...
grep 'ref=6a28f3c571728f1d3fea4ae7547539555277aa67' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=6a28f3c571728f1d3fea4ae7547539555277aa67[a-z0-9]*/${root}\/\2/g" "${file}"
done
