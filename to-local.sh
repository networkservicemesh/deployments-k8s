...
grep '?ref=1c2bdb1e959ace56e909c8b0d57cc9932f70a6e1' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=1c2bdb1e959ace56e909c8b0d57cc9932f70a6e1/${root}\/\2/g" "${file}"
done
