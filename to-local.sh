...
grep '?ref=869c369554c0387b9a8d7fa5e002354edf90f1ac' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=869c369554c0387b9a8d7fa5e002354edf90f1ac/${root}\/\2/g" "${file}"
done
