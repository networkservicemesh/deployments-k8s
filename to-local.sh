...
grep 'ref=299e9cd005d658013c4e1f57cffbfa5a6096f406' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=299e9cd005d658013c4e1f57cffbfa5a6096f406[a-z0-9]*/${root}\/\2/g" "${file}"
done
