...
grep 'ref=1b21796131dec8aad9013703d14425a74dcb504b' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=1b21796131dec8aad9013703d14425a74dcb504b[a-z0-9]*/${root}\/\2/g" "${file}"
done
