...
grep 'ref=0ce471897566e355c3162070d41f3861331cbaac' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=0ce471897566e355c3162070d41f3861331cbaac[a-z0-9]*/${root}\/\2/g" "${file}"
done
