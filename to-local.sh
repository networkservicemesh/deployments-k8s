...
grep 'ref=7f15e8ee8075bd9de4f015a695a6e6a3243183d9' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=7f15e8ee8075bd9de4f015a695a6e6a3243183d9[a-z0-9]*/${root}\/\2/g" "${file}"
done
