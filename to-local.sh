...
grep 'ref=5eb2f12e0a69558304f01de8e64033756941979f' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=5eb2f12e0a69558304f01de8e64033756941979f[a-z0-9]*/${root}\/\2/g" "${file}"
done
