...
grep 'ref=01c3d324af19d510de89fe8695d8016c3db3899f' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=01c3d324af19d510de89fe8695d8016c3db3899f[a-z0-9]*/${root}\/\2/g" "${file}"
done
