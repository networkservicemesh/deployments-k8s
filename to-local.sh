...
grep 'ref=38cdc55a9673c4b8b01387e5f463da8f2134e7e3' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=38cdc55a9673c4b8b01387e5f463da8f2134e7e3[a-z0-9]*/${root}\/\2/g" "${file}"
done
