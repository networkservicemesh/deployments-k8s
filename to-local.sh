...
grep 'ref=a28022c1b5e5fae4a27a72147009be31c1d5966f' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=a28022c1b5e5fae4a27a72147009be31c1d5966f[a-z0-9]*/${root}\/\2/g" "${file}"
done
