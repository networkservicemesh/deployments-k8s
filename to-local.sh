...
grep 'ref=9acb86392ed6423005a882af7cd8dfa73c47f0af' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=9acb86392ed6423005a882af7cd8dfa73c47f0af[a-z0-9]*/${root}\/\2/g" "${file}"
done
