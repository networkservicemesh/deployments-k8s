...
grep 'ref=40b3635a022883596c89d5a3d2910d0a7a1be5bb' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=40b3635a022883596c89d5a3d2910d0a7a1be5bb[a-z0-9]*/${root}\/\2/g" "${file}"
done
