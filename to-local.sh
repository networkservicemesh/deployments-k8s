...
grep 'ref=2dfa10d52d2abe34be45fb48601a2884944f5271' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=2dfa10d52d2abe34be45fb48601a2884944f5271[a-z0-9]*/${root}\/\2/g" "${file}"
done
