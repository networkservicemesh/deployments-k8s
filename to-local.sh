...
grep 'ref=71294227f48fbc730a6a245a4841080980dffb54' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=71294227f48fbc730a6a245a4841080980dffb54[a-z0-9]*/${root}\/\2/g" "${file}"
done
