...
grep 'ref=fb18d3fedd5dfa311c6d1d686a2f843c453a923a' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=fb18d3fedd5dfa311c6d1d686a2f843c453a923a[a-z0-9]*/${root}\/\2/g" "${file}"
done
