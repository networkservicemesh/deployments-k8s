...
grep 'ref=b4e64d2c715cb540ea58400cd042412e9d67d42e' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=b4e64d2c715cb540ea58400cd042412e9d67d42e[a-z0-9]*/${root}\/\2/g" "${file}"
done
