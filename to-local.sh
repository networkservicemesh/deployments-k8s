...
grep 'ref=eab388600b1e6eb82cb777ddf58fbe15f904256a' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=eab388600b1e6eb82cb777ddf58fbe15f904256a[a-z0-9]*/${root}\/\2/g" "${file}"
done
