...
grep 'ref=b3ba4b5dbc210797238c45a782ed8626813c6592' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=b3ba4b5dbc210797238c45a782ed8626813c6592[a-z0-9]*/${root}\/\2/g" "${file}"
done
