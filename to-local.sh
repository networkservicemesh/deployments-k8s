...
grep '?ref=b522b827fb765f73a70ffb4c8863c878170237da' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=b522b827fb765f73a70ffb4c8863c878170237da/${root}\/\2/g" "${file}"
done
