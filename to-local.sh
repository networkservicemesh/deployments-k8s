...
grep 'ref=9948922016185ac53315d0f847b231ae8db14ac7' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=9948922016185ac53315d0f847b231ae8db14ac7[a-z0-9]*/${root}\/\2/g" "${file}"
done
