...
grep '?ref=448af3192445b5b6ba4b6cbcea08987735093486' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=448af3192445b5b6ba4b6cbcea08987735093486/${root}\/\2/g" "${file}"
done
