...
grep '?ref=eedf4a55be91a9102f10c0832bd4240a55cc2f7c' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=eedf4a55be91a9102f10c0832bd4240a55cc2f7c/${root}\/\2/g" "${file}"
done
