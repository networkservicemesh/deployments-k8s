...
grep '?ref=5073901e2b85196d825e390abd720bef1e0c335d' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=5073901e2b85196d825e390abd720bef1e0c335d/${root}\/\2/g" "${file}"
done
