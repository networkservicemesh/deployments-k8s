...
grep 'ref=v1.3.0-rc.1' -rl examples/* | while IFS= read -r file; do
  root="$(get_root "$file")"
  sedi -E "s/(https:\/\/)?github.com\/networkservicemesh\/deployments-k8s\/(.*)\?ref=v1.3.0-rc.1[a-z0-9]*/${root}\2/g" "${file}"
done
